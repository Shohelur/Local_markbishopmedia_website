...
const targetStr = '</div>\n\n</section>';
const graphicHtml = `</div>

<!-- LIVE DATA STREAM GRAPHIC (RIGHT SIDE) -->
<div id="engine-status-widget" style="position:absolute; bottom:30px; right:40px; width:260px; border: 1px solid rgba(184, 255, 87, 0.2); background: rgba(2, 6, 15, 0.75); backdrop-filter: blur(12px); padding: 18px; border-radius: 12px; z-index: 20; display:flex; flex-direction:column; gap: 14px; box-shadow: 0 0 30px rgba(184, 255, 87, 0.1);">
  <style>
...
html = html.replace(targetStr, graphicHtml);

// if for some reason the exact target string wasn't found due to spacing, fallback to index
if (!html.includes('id="engine-status-widget"')) {
  const endIndex = html.lastIndexOf('</section>');
  html = html.substring(0, endIndex) + graphicHtml.replace('</div>\n\n</section>', '') + '</section>' + html.substring(endIndex + 10);
}

fs.writeFileSync('final-website-master.html', html, 'utf8');
console.log('Graphic widget added!');
