...
# 1. Wrap Phase 10, 11, 13 in .unified-ocean-zone
html = html.replace('<section id="phase10-comparison">', '<div class="unified-ocean-zone" id="unified-ocean-zone">\n  <canvas id="unified-ocean-canvas"></canvas>\n  <section id="phase10-comparison">')
html = html.replace('</footer>\n\n  <!-- ==============================================\n       END PHASE 13', '</footer>\n  </div> <!-- END UNIFIED OCEAN ZONE -->\n\n  <!-- ==============================================\n       END PHASE 13')

# 2. Remove the old canvas from Phase 13
html = re.sub(r'<canvas id="p13-ribbon-canvas"></canvas>\s*', '', html)

# 3. Inject CSS for the Unified Zone and Cards
css_injection = """
  /* =========================================
  ...
  .unified-ocean-zone {
    ...
    width: 100%;
    overflow: hidden;
    background: #02050f; /* Base dark color */
  }
  ...
  /* Make sure cards block the particles from showing through text */
  .p10-card {
...
# Find the closing </style> and inject right before it
html = html.replace('</style>', css_injection + '\n</style>')

# 4. Replace the old JS engine with the Unified Engine
js_pattern = r"<script>\s*// --- V4: FLUID PARTICLE OCEAN \(REPULSION PHYSICS\).*?</script>"
unified_js = """<script>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Physics Constants (Scaled up for massive area)
  const NUM_PARTICLES = window.innerWidth > 768 ? 1400 : 500;
  const MOUSE_RADIUS = 250; // Larger repulsion area
  ...
  function connectParticles() {
    for (let a = 0; a < particles.length; a++) {
      for (let b = a + 1; b < particles.length; b++) {
        // Optimization: Only check distance if Y coordinates are somewhat close
...
if re.search(js_pattern, html, flags=re.DOTALL):
    html = re.sub(js_pattern, unified_js, html, flags=re.DOTALL)
    with open(master_path, 'w', encoding='utf-8') as f:
        f.write(html)
    print("Unified Fluid Ocean Engine injected successfully across Phase 10, 11, 13!")
else:
    print("Error: Could not find the V4 script block.")
