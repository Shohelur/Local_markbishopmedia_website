...
has_generated_client_gitignore() {
  ...
  [[ -f "$target" ]] || return 1
  grep -Fqx "$CLIENT_GITIGNORE_START" "$target" && grep -Fqx "$CLIENT_GITIGNORE_END" "$target"
}

client_gitignore_manages_skill() {
  local client_dir="$1"
  local skill_name="$2"
  local target="${client_dir}/.gitignore"

  if ! has_generated_client_gitignore "$client_dir"; then
    return 0
  fi

  grep -Fqx ".claude/skills/${skill_name}/*" "$target"
}

generate_client_gitignore_block() {
  local excluded_skills="${1:-}"
  local root_skill
  local skill_name
...
# Root-copied shared skill catalog.
.claude/skills/_catalog/*

# Root-copied shared skills. Client-only skill folders are not listed here.
EOF
  ...
  if [[ -d "${PROJECT_DIR}/.claude/skills" ]]; then
    for root_skill in "${PROJECT_DIR}/.claude/skills"/*/; do
      ...
      skill_name_in_list "$skill_name" "$excluded_skills" && continue

      echo ".claude/skills/${skill_name}/*"
      echo "!.claude/skills/${skill_name}/SKILL.local.md"
    done
  fi
...
write_client_gitignore() {
  local client_dir="$1"
  local excluded_skills="${2:-}"
  local target="${client_dir}/.gitignore"
  local block_file
  local merged_file

  block_file="$(mktemp)"
  generate_client_gitignore_block "$excluded_skills" > "$block_file"

  if [[ -f "$target" ]]; then
    if grep -Fqx "$CLIENT_GITIGNORE_START" "$target" && grep -Fqx "$CLIENT_GITIGNORE_END" "$target"; then
      merged_file="$(mktemp)"
...
for CLIENT_DIR in "${CLIENTS_DIR}"/*/; do
  ...
  if [[ -d "${PROJECT_DIR}/.claude/skills" ]]; then
    ...
    for root_skill in "${PROJECT_DIR}/.claude/skills"/*/; do
      ...
      if [[ -d "$client_skill_dir" ]] && ! client_gitignore_manages_skill "$CLIENT_DIR" "$skill_name"; then
        COLLIDING_SKILLS="${COLLIDING_SKILLS} ${skill_name}"
        continue
      fi
    ...
    if [[ -n "$COLLIDING_SKILLS" ]]; then
      echo "  Warning: preserved client-owned skill folder(s) that now match root skill name(s):${COLLIDING_SKILLS}"
      echo "  Rename the client skill or move shared changes into SKILL.local.md before syncing those root skill(s)."
    fi
  fi
  ...
  write_client_gitignore "$CLIENT_DIR" "$COLLIDING_SKILLS"
...
if [[ $CLIENT_COUNT -eq 0 ]]; then
  ...
  echo ""
  echo "What was synced: client instruction files, managed shared skills, scripts, settings, hooks, cron templates, client .gitignore."
  echo "What was NOT touched: brand_context, memory, learnings, projects, .env, cron jobs, client-only skills, SKILL.local.md."
fi
