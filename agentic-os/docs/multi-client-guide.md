...
- shared skill base files copied from root
...
Client-specific changes to a shared skill should go in `SKILL.local.md` next to that skill's `SKILL.md`. During updates, Agentic OS replaces the shared base skill files from root but preserves each client's `SKILL.local.md` files and `_catalog/installed.json` skill selection state. The shared base files are ignored in git; `SKILL.local.md` stays trackable.
