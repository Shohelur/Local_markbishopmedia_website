...
- 2026-09-13 [CRITICAL BEHAVIOR]: NEVER execute destructive git commands (like `git reset --hard`, `git checkout .`, `git clean`) under any circumstances without explicit user permission. Doing so deletes uncommitted user work. Always commit and push to GitHub immediately after successfully completing a task to prevent data loss.
- 2026-09-13 [CRITICAL BEHAVIOR]: When the user repeatedly gives a specific instruction (e.g., "Fix transparency at the IMAGE ASSET level, NOT with CSS"), DO NOT repeatedly attempt to solve it using the forbidden method. Follow the user's explicit constraints on the first attempt.
- 2026-09-13 [Phase 7 Industry Verticals]: Infinite Horizontal Marquee vs Bento Grid. When displaying a long list of categories (like 8-10 industries), a Bento Grid takes up massive vertical screen space (~600px+) and clutters the UI. A single-line infinite scrolling Marquee (ticker) with pause-on-hover takes only ~120px height, conveys the same information elegantly, and creates a much more premium, fast-moving flow.
- 2026-09-14 [Phase 10 Copy Injection]: html.replace() or exact string matching in Python fails silently if HTML contains hidden newlines or HTML-encoded em-dashes. ALWAYS use e.sub with e.DOTALL and non-greedy matching .*? across multiple lines.
- 2026-09-14 [Fireworks UI]: Switched from custom CSS sparks to canvas-confetti CDN for Phase 10 victory sequence. Much higher visual quality and easier to control origin points relative to DOM rects.
