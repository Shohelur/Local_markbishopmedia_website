...
| `viz-image-gen` | Interactive visual direction and image generation via GPT Image or Gemini | `OPENAI_API_KEY` or `GEMINI_API_KEY` |
...
bash scripts/remove-skill.sh viz-image-gen # Remove a skill
```
...
Updates never overwrite your root or client data. For clients, Agentic OS syncs managed shared skills, scripts, hooks, settings, and cron templates from the root. Client context, brand data, memory, projects, cron jobs, `.env`, `.claude/settings.local.json`, client-only skills, and `SKILL.local.md` files are preserved.
...
To use a different update source or branch, set `AGENTIC_OS_UPSTREAM_SLUG` and `AGENTIC_OS_UPSTREAM_BRANCH` in the root `.env` file. The updater intentionally ignores values inherited from the terminal or another process so stale settings cannot override the project configuration. Missing or empty values default to `simonc602/agentic-os` and `main`.

If an update stops unexpectedly after it has started protecting local files, Agentic OS prints `Your files are safe in:` with exact backup paths and writes a report to `.backup/update-recovery-<timestamp>.txt`. To show the latest report later, run:

```bash
bash scripts/update-recovery.sh --latest
```
...
Semantic recall is **optional**. Without it, Tier 0 recall (`MEMORY.md` + today's log) still works. Once enabled, semantic search, expand, transcript drill-down, and stronger citations all become available — driven automatically by the `meta-memory-recall` skill, or run manually with `npm run memory:expand` / `npm run memory:transcript`. Memory is also scope-isolated (system/team/client/private), so a search never surfaces another client's or another user's private memory. See [`docs/memory-retrieval.md`](docs/memory-retrieval.md) for the full recall ladder and scope model.

### Import prior Claude Code sessions (optional)

If you used Claude Code before installing Agentic OS, you can backfill that history into the same memory store so recall works from day one. Setup offers this once memory is ready, or run it yourself:

```bash
cd command-centre
npm run memory:import-sessions -- --dry-run    # detect + count, no writes
npm run memory:import-sessions                  # interactive: pick source, size, confirm
```

It is user-controlled and never silent. Choose the source (current workspace, all global Claude Code history under `~/.claude/projects/`, existing `context/transcripts/` archives, or an explicit `--source`), and how many to import newest-first (`--all`, `--sessions N`, `--days N`, or `--since/--until`). Summarization uses your Claude subscription usage and large histories take time, so importing fewer now and more later is fine. Re-running never duplicates — a ledger under `.command-centre/memory-import/` and per-block source hashes keep imports idempotent, and the importer indexes the result so it is searchable as soon as indexing finishes. Full details: [`docs/memory-retrieval.md`](docs/memory-retrieval.md).
...
Each client also gets a generated `.gitignore`. Commit client data such as `AGENTS.md`, `CLAUDE.md`, `.gitignore`, `brand_context/`, `context/`, `projects/`, `cron/jobs/`, client-only skills, and `SKILL.local.md` overrides. Root-copied scripts, shared skill base files, hooks, shared settings, and cron templates are regenerated from the root and stay ignored.
...
Team OS adds server-managed accounts, Teams, client access, shared memory,
skills, and secrets to Command Centre. One account can move between several
Teams, while existing chats, tasks, and agents keep the Team and client where
they started. A shared installation supports one active human account at a time
and keeps each account's local profile separate.

Each server represents one company. The Company Owner can manage every Team.
Company Admins see the company-level **Teams** section and receive Full access
only to the Teams granted to them, unless they already have a Team Owner or Team
Admin membership. Promoting an existing platform account happens immediately.
An invitation link is created only when that person still needs to create an
account.

See the [Team OS user guide](docs/team-os-user-guide.md) for sign-in, Team
switching, history, safe logout, profile recovery, and Solo compatibility.
...
├── team_context/          <- Team instructions and shared Team OS context
...
For clients, Agentic OS does sync shared system files from the root, including managed shared skills, scripts, hooks, settings, and cron templates. Client context, brand data, memory, projects, cron jobs, `.env`, `.claude/settings.local.json`, client-only skills, and `SKILL.local.md` files are preserved. If a client-only skill later has the same folder name as a new root skill, the update preserves the client-owned folder and warns you instead of replacing it silently.
