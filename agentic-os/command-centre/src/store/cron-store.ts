  ...
  try {
    ...
    const stored =
      typeof window !== "undefined"
        ? localStorage.getItem(profileStorageKey(scopedKey))
        : null;
  ...
  try {
    if (typeof window !== "undefined") {
      localStorage.setItem(
        profileStorageKey(getScopedStorageKey(CRON_ORDER_KEY, clientId)),
...
function savePinnedSlugs(clientId: string | null, slugs: string[]) {
  try {
    if (typeof window !== "undefined") {
      localStorage.setItem(
        profileStorageKey(getScopedStorageKey(CRON_PINNED_KEY, clientId)),
        JSON.stringify(slugs)
      );
    }
  } catch {
