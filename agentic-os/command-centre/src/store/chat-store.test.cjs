      ...
      "@/lib/api-error": {
        async readApiError(response, fallbackMessage) {
          try {
            ...
            // Match the production helper: malformed responses use the fallback.
          }
...
test("loadOrCreateConversation restores the remembered conversation for the current client scope", async () => {
  ...
  global.fetch = async (input) => {
    ...
    if (url.endsWith("/api/chat/conversations/conv-remembered?defaultScope=1")) {
  ...
  try {
    ...
    assert.deepEqual(calls, [
      "/api/chat/conversations/conv-remembered?defaultScope=1",
...
test("loadOrCreateConversation falls back to the latest active conversation when the remembered id is stale", async () => {
  ...
  global.fetch = async (input) => {
    calls.push(String(input));
    const url = String(input);
    if (url.endsWith("/api/chat/conversations/conv-stale?defaultScope=1")) {
      return createJsonResponse({ error: "not found" }, 404);
    }
    if (url === "/api/chat/conversations?clientId=client-1&status=active&defaultScope=1") {
  ...
  try {
    ...
    assert.deepEqual(calls, [
      "/api/chat/conversations/conv-stale?defaultScope=1",
      "/api/chat/conversations?clientId=client-1&status=active&defaultScope=1",
