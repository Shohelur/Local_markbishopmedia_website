...
function loadColumnPrefs(): ColumnPrefs {
  try {
    const raw = localStorage.getItem(profileStorageKey(STORAGE_KEY));
    if (raw) {
...
function saveColumnPrefs(prefs: ColumnPrefs) {
  try {
    localStorage.setItem(profileStorageKey(STORAGE_KEY), JSON.stringify(prefs));
  } catch { /* ignore */ }
}
...
export default function HistoryPage() {
  ...
  const tabs: { key: string; label: string; icon: typeof Activity; href: string }[] = [
    ...
    { key: "scheduled", label: "Scheduled", icon: Clock, href: "/scheduled" },
    { key: "skills", label: "Skills", icon: Cpu, href: "/skills" },
