...
export async function POST(
  ...
  try {
    ...
    const newParentScope = await captureNewWorkScope(requestedClientId);
    const newParentTeamId = getTeamIdForWorkScope(newParentScope.scope);
    const scopeFilter = newParentScope.scope.mode === "team"
    ...
    if (body.parentTaskId) {
      ...
      parentTask = db
        ...
        .get(
          ...
          requestedClientId,
          requestedClientId,
          ...(newParentTeamId ? [newParentTeamId] : []),
        ) as Task | undefined;
    }
    ...
    let parentId: string | null = parentTask?.id ?? null;
    ...
    if (!parentId) {
      ...
      db.prepare(
        `INSERT INTO tasks (id, title, description, status, level, parentId, projectSlug, columnOrder, createdAt, updatedAt, clientId, workScope, needsInput, permissionMode, executionPermissionMode)
        ...
        now,
        now,
        newParentScope.clientId,
        newParentScope.serialized,
      );
    ...
    if (!parentTask) {
      return NextResponse.json({ error: "Goal could not be resolved" }, { status: 500 });
    }
