...
function tryRegisterFile(
  ...
  const relativePath = path.relative(config.agenticOsDir, path.resolve(filePath));
  if (!relativePath || relativePath.startsWith("..") || path.isAbsolute(relativePath)) return false;
  if (!isMaterializedPathAccessible(filePath)) return false;
...
export async function GET(
  ...
  const task = db.prepare("SELECT id, startSnapshot, projectSlug, clientId, workScope FROM tasks WHERE id = ?").get(id) as
    | { id: string; startSnapshot: string | null; projectSlug: string | null; clientId: string | null; workScope: string | null }
    | undefined;
  if (!task) {
  ...
  outputs = outputs.filter((file) => isMaterializedPathAccessible(file.filePath));
