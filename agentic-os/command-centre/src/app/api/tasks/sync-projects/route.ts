...
import { getGsdSyncedTaskStatus } from "@/lib/task-status-transitions";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const clientId = normalizeClientId(searchParams.get("clientId"));
    ...
    if (!fs.existsSync(briefsDir)) {
      return NextResponse.json({ synced: 0, skippedArchived: 0 });
    }

    const db = getDb();
    const captured = await captureNewWorkScope(clientId);
    const capturedTeamId = getTeamIdForWorkScope(captured.scope);
    ...
    for (const entry of entries) {
      ...
      const parentScopeFilter = captured.scope.mode === "team"
      ...
      let parentTask = db.prepare(
        ...
        captured.clientId,
        captured.clientId,
        ...(capturedTeamId ? [capturedTeamId] : []),
      ) as Task | undefined;

      if (parentTask) {
        inheritWorkScope(parentTask);
      }
      ...
      if (!parentTask) {
        ...
        db.prepare(
          `INSERT INTO tasks (id, title, description, status, level, parentId, projectSlug, columnOrder, createdAt, updatedAt, clientId, workScope, permissionMode, executionPermissionMode)
      ...
      if (level === "gsd" && parentTask) {
        ...
        if (planningDir && fs.existsSync(roadmapPath) && isMaterializedPathAccessible(roadmapPath)) {
          ...
          for (const phase of phases) {
            for (let stepIdx = 0; stepIdx < GSD_STEPS.length; stepIdx++) {
              ...
              const status = getGsdSyncedTaskStatus(
                phase.status,
                step,
                phase.status === "in-progress" && step === "discuss",
              );
              ...
              db.prepare(
                `INSERT INTO tasks (id, title, description, status, level, parentId, projectSlug, columnOrder, createdAt, updatedAt, clientId, workScope, needsInput, phaseNumber, gsdStep, permissionMode, executionPermissionMode, model, thinkingEffort)
                 VALUES (?, ?, ?, ?, 'task', ?, ?, ?, ?, ?, ?, ?, 0, ?, ?, ?, ?, ?, ?)`
              ).run(
                taskId, title,
                ...
                now, now, inheritedScope.clientId, inheritedScope.serialized,
      ...
      if (level === "project" && parentTask) {
        ...
        if (existingCount.cnt === 0) {
          ...
          for (const title of deliverables) {
            ...
            const taskId = crypto.randomUUID();
            db.prepare(
              `INSERT INTO tasks (id, title, status, level, parentId, projectSlug, columnOrder, createdAt, updatedAt, clientId, workScope, needsInput, permissionMode, executionPermissionMode, model, thinkingEffort)
    ...
    return NextResponse.json({ synced, skippedArchived });
  } catch (error) {
