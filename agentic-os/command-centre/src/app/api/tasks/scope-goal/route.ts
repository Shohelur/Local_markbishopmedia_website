...
export async function POST(request: NextRequest) {
  try {
    ...
    const captured = await captureNewWorkScope(normalizedClientId);
    const teamId = getTeamIdForWorkScope(captured.scope);
    const gathered = await gatherContext(goal, captured.clientId, teamId);
    ...
    const taskScopeCondition = captured.scope.mode === "team"
    ...
    const activeTasks = db
      ...
      .all(...(teamId ? [teamId] : [])) as ActiveTask[];
