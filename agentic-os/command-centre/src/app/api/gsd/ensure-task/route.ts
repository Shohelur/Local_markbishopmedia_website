...
export async function POST(request: Request) {
  try {
    ...
    const db = getDb();
    const currentScope = await captureNewWorkScope(null);
    const currentTeamId = getTeamIdForWorkScope(currentScope.scope);
    const scopeFilter = currentScope.scope.mode === "team"
    ...
    // Archived Goals remain immutable even when their planning files were
    // removed or are temporarily unavailable.
    const archivedParent = db.prepare(
      `SELECT * FROM tasks WHERE level = 'gsd' AND parentId IS NULL AND clientId IS NULL AND projectSlug = ? ${scopeFilter} ORDER BY createdAt DESC LIMIT 1`,
    ).get(resolvedSlug, ...(currentTeamId ? [currentTeamId] : [])) as Task | undefined;
    if (archivedParent && isTaskArchived(db, archivedParent.id)) {
      return NextResponse.json({
        synced: false,
        reason: "archived",
        parentTaskId: archivedParent.id,
      });
    }
    ...
    if (fs.existsSync(candidateBrief) && isMaterializedPathAccessible(candidateBrief)) {
    ...
    if (projectSlug) {
      parentTask = db.prepare(
        `SELECT * FROM tasks WHERE level = 'gsd' AND parentId IS NULL AND clientId IS NULL AND projectSlug = ? ${scopeFilter} ORDER BY createdAt DESC LIMIT 1`
      ).get(projectSlug, ...(currentTeamId ? [currentTeamId] : [])) as Task | undefined;
    }

    if (!projectSlug && !parentTask) {
      // Also check by title match as fallback
      parentTask = db.prepare(
        `SELECT * FROM tasks WHERE level = 'gsd' AND parentId IS NULL AND clientId IS NULL AND title LIKE ? ${scopeFilter} ORDER BY createdAt DESC LIMIT 1`
      ).get(`%${projectName}%`, ...(currentTeamId ? [currentTeamId] : [])) as Task | undefined;
    }

    if (parentTask) {
      inheritWorkScope(parentTask as Task & { workScope?: string | null });
    }
    if (parentTask && isTaskArchived(db, parentTask.id)) {
      return NextResponse.json({
        synced: false,
        reason: "archived",
    ...
    const parentStatus = allComplete ? "review" : "running";
    ...
    if (!parentTask) {
      ...
      db.prepare(
        `INSERT INTO tasks (id, title, description, status, level, parentId, projectSlug, columnOrder, createdAt, updatedAt, costUsd, tokensUsed, durationMs, activityLabel, errorMessage, startedAt, completedAt, clientId, workScope, needsInput, phaseNumber, gsdStep, cronJobSlug, permissionMode, executionPermissionMode)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(
        ...
        task.startedAt, task.completedAt, task.clientId, currentScope.serialized, 0,
    ...
    let createdCount = 0;
    const inheritedParentScope = inheritWorkScope(
      parentTask as Task & { workScope?: string | null },
    );
    ...
    for (const phase of phases) {
      for (let stepIdx = 0; stepIdx < GSD_STEPS.length; stepIdx++) {
        ...
        const targetStatus = getGsdSyncedTaskStatus(
          phase.status,
          step,
          (step === "discuss" || step === "plan") && hasPlanForPhase(phase.number),
        );

        if (existing) {
          // Only update if status needs to change (don't overwrite manual changes for active tasks)
          if (shouldApplySyncedTaskStatus(existing.status, targetStatus)) {
        ...
        const task: Task = {
          ...
          workScope: inheritedParentScope.scope,
          needsInput: false,
        ...
        db.prepare(
          ...
          task.createdAt, task.updatedAt, task.costUsd, task.tokensUsed,
          task.durationMs, task.activityLabel, task.errorMessage,
          task.startedAt, task.completedAt, task.clientId, inheritedParentScope.serialized, 0,
