...
export async function PATCH(request: NextRequest) {
  try {
    ...
    const parentTask = db
      .prepare(
        `SELECT id, clientId, workScope FROM tasks
         WHERE projectSlug = ?
      ...
      ) as { id: string; clientId: string | null; workScope: string | null } | undefined;
    if (parentTask) {
      readWorkScopeFromRow(parentTask);
    ...
    assertMaterializedPathAccessible(roadmapPath);
    ...
    registerMaterializedFiles([roadmapPath], { scope: "team", kind: "gsd-roadmap" });
    ...
    try {
      const existing = db
        .prepare(
          "SELECT * FROM tasks WHERE parentId = ? AND phaseNumber = ? AND gsdStep IS NOT NULL"
        )
        .all(parentTask?.id ?? "", phaseNumber) as Task[];
      ...
      if (existing.length > 0 && parentTask) {
