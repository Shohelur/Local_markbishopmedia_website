...
export async function GET(request: NextRequest) {
  try {
    ...
    if (fileParam) {
      ...
      if (!fullPath.startsWith(planningDir) || !fs.existsSync(fullPath) || fs.statSync(fullPath).isDirectory() || !isMaterializedPathAccessible(fullPath)) {
    ...
    if (phaseParam) {
      ...
      const files = fs.readdirSync(dirPath)
        ...
        .filter((f) => isMaterializedPathAccessible(path.join(dirPath, f)))
        .map((f) => ({
          name: f,
    ...
    for (const name of topLevelFiles) {
      ...
      if (fs.existsSync(fp) && isMaterializedPathAccessible(fp)) {
        projectFiles.push({
          name,
          relativePath: name,
    ...
    if (fs.existsSync(phasesDir)) {
      ...
      for (const entry of entries) {
        ...
        if (files.length > 0) phases.push({ phaseNumber, dirName: entry.name, files });
      }
    }
    ...
    if (fs.existsSync(researchDir)) {
      for (const f of fs.readdirSync(researchDir).filter((f) => f.endsWith(".md"))) {
        if (!isMaterializedPathAccessible(path.join(researchDir, f))) continue;
        researchFiles.push({
          name: f,
