...
const TASK_LOG_SELECT = `
  SELECT id, type, timestamp, content, toolName, toolArgs, toolResult, toolUseId, parentToolUseId, isCollapsed
  FROM task_logs
`;
...
export function insertTaskLog(
  ...
  db.prepare(
    `INSERT INTO task_logs (
      id, taskId, type, timestamp, content, toolName, toolArgs, toolResult, toolUseId, parentToolUseId, isCollapsed
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    entry.id,
    taskId,
