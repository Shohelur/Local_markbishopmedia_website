...
function pollCronTasks() {
  try {
    const profileKey = getActiveLocalProfileDescriptor().profileKey;
    const db = getDb();
    ...
    for (const task of cronTasks) {
      ...
      const scopedTaskId = `${profileKey}:${task.id}`;
      const prevState = state.knownTaskStates.get(scopedTaskId);
      ...
      state.knownTaskStates.set(scopedTaskId, stateKey);
    }

    // Clean up old entries (tasks no longer in the last 50)
    const activeIds = new Set(cronTasks.map((t) => `${profileKey}:${t.id}`));
    for (const id of state.knownTaskStates.keys()) {
      if (id.startsWith(`${profileKey}:`) && !activeIds.has(id)) {
...
export function startCronTaskSync() {
  ...
  try {
    ...
    for (const t of existing) {
      state.knownTaskStates.set(`${profileKey}:${t.id}`, `${t.status}:${t.updatedAt}`);
    }
  } catch {
...
export function clearCronTaskSyncProfile(profileKey: string): void {
  const state = getState();
  for (const key of state.knownTaskStates.keys()) {
    if (key.startsWith(`${profileKey}:`)) state.knownTaskStates.delete(key);
  }
}
