...
export interface FileSnapshot {
  [relativePath: string]: {
    ...
    content?: string;
    digest?: string;
    unavailableReason?: "binary" | "oversized" | "unreadable";
  };
}
...
function digestFile(filePath: string): string {
  ...
  try {
    ...
    let bytesRead = 0;
    do {
      bytesRead = fs.readSync(handle, buffer, 0, buffer.length, offset);
      if (bytesRead > 0) {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory()) {
        ...
        try {
          ...
          const record: FileSnapshot[string] = { size: stat.size, digest: digestFile(fullPath) };
          if (isTextFile(entry.name) && stat.size < MAX_CONTENT_SIZE) {
            record.content = fs.readFileSync(fullPath, "utf-8");
          } else {
            record.unavailableReason = isTextFile(entry.name) ? "oversized" : "binary";
          }
          snapshot[relPath] = record;
        } catch {
          snapshot[relPath] = { size: 0, unavailableReason: "unreadable" };
        }
      }
    }
  };
