...
export function loadPromptTags(
  ...
  try {
    ...
    if (trustMode === "team") {
      ...
      return discoverBriefTags(agenticOsDir);
    }

    // 1. Shared team tags, then private user/client tags. Later tags win.
    const teamManual = readPromptTagsFile(path.join(config.agenticOsDir, "team_context", "prompt-tags.md"));
    const rootManual = readPromptTagsFile(path.join(config.agenticOsDir, "context", "prompt-tags.md"));
    const scopedManual = clientId
      ? readPromptTagsFile(path.join(agenticOsDir, "context", "prompt-tags.md"))
      : [];
