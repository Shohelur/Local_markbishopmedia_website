...
export function getConfig(): Config {
  ...
  cachedConfig = { agenticOsDir, dataDir, dbPath };
  return cachedConfig;
}
...
export function getClientAgenticOsDir(clientId: string | null): string {
  ...
  if (!clientId || clientId === "root") return config.agenticOsDir;
  if (path.isAbsolute(clientId) || clientId.includes("/") || clientId.includes("\\") || clientId.includes("..")) {
    throw new Error("Invalid client id");
  }
  return path.join(config.agenticOsDir, "clients", clientId);
}

function isSafeClientSlug(value: string): boolean {
  return (
    value.length > 0 &&
...
/**
 * Infer the Agentic OS client slug for a cwd under clients/{slug}/...
 * Returns null for the root workspace, command-centre, unknown paths, and
 * paths outside this workspace.
 */
export function detectClientIdFromCwd(
  cwd: string | null | undefined,
  agenticOsDir: string = getConfig().agenticOsDir,
): string | null {
  if (!cwd || typeof cwd !== "string") return null;

  const root = path.resolve(agenticOsDir);
  ...
  if (parts[0] !== "clients" || !parts[1] || !isSafeClientSlug(parts[1])) {
    return null;
  }

  const clientDir = path.join(root, "clients", parts[1]);
  try {
    if (!fs.statSync(clientDir).isDirectory()) return null;
  } catch {
    return null;
  }
