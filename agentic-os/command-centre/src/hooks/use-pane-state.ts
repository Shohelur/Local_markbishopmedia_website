...
export function replacePaneTaskInState(
  state: PaneState,
  paneId: string,
  task: { id: string; title?: string },
): PaneState {
  if (paneId === MAIN_PANE_ID || !state.openPanes.some((pane) => pane.id === paneId)) {
    return state;
  }

  return {
    ...state,
    openPanes: state.openPanes.map((pane) =>
      pane.id === paneId
        ? { ...pane, taskId: task.id }
        : pane,
    ),
...
export function removeUnavailableTerminalPanes(state: PaneState): PaneState {
  const openPanes = state.openPanes.filter((pane) => pane.type !== "terminal");
  const availablePaneIds = new Set([MAIN_PANE_ID, ...openPanes.map((pane) => pane.id)]);
  const visiblePaneIds = state.visiblePaneIds.filter((paneId) => availablePaneIds.has(paneId));
  const normalizedVisiblePaneIds = visiblePaneIds.length > 0 ? visiblePaneIds : [MAIN_PANE_ID];
  const activePaneId = normalizedVisiblePaneIds.includes(state.activePaneId)
    ? state.activePaneId
    : normalizedVisiblePaneIds[0];

  return {
    ...state,
    openPanes,
    visiblePaneIds: normalizedVisiblePaneIds,
    activePaneId,
  };
}
...
function loadPersistedState(key: string): PaneState | null {
  ...
  try {
    const raw = localStorage.getItem(profileStorageKey(STORAGE_PREFIX + key));
    if (!raw) return null;
    ...
    if (!Array.isArray(parsed.visiblePaneIds)) {
      ...
      return removeUnavailableTerminalPanes(ensureNotEmpty({
    ...
    return removeUnavailableTerminalPanes(ensureNotEmpty(parsed as PaneState));
  } catch {
    return null;
  }
}
...
function persistState(key: string, state: PaneState) {
  ...
  try {
    localStorage.setItem(profileStorageKey(STORAGE_PREFIX + key), JSON.stringify(state));
  } catch {}
}
...
export function usePaneState(persistKey?: string) {
  ...
  const handleReplacePaneTask = useCallback((paneId: string, task: { id: string; title?: string }) => {
    setPaneState((prev) => replacePaneTaskInState(prev, paneId, task));
  }, []);

  const resetPaneState = useCallback(() => {
    setPaneState(INITIAL_STATE);
    if (persistKey) {
      try { localStorage.removeItem(profileStorageKey(STORAGE_PREFIX + persistKey)); } catch {}
    }
  }, [persistKey]);
