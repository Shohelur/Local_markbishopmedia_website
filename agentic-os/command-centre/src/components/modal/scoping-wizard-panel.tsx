...
function storageKeyForGoal(goal: string): string {
  ...
  return profileStorageKey(`${STORAGE_KEY_PREFIX}${h}`);
}
...
function LoadedContextCard({
  ...
  const memoryCount = sources.filter((s) => s.kind === "memory").length;
  const summaryParts: string[] = [];
  if (brandCount > 0) summaryParts.push(`${brandCount} brand file${brandCount === 1 ? "" : "s"}`);
  if (memoryCount > 0) summaryParts.push(`${memoryCount} memory result${memoryCount === 1 ? "" : "s"}`);
    ...
    >
      ...
      >
        {sources.map((s) => (
          ...
          >
            {s.kind === "brand" ? "📄" : s.kind === "memory" ? "◈" : "🔗"} {s.label}
          </span>
        ))}
      </div>
