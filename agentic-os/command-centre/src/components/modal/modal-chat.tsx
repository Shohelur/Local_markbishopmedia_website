...
import { ArrowUp, Bot, FileText, Search, Terminal, Globe, Wrench, ListChecks, ChevronDown, ChevronRight, Check, Circle, MessageSquare } from "lucide-react";
...
function AgentLifecycleBlock({
  ...
  const hasActive = isCreated && events.some((event) => activeKeys.has(event.toolUseKey));
  const label = isCreated
    ? formatCreatedAgentsLabel(events.length, hasActive)
    : formatClosedAgentsLabel(events.length);
  ...
  return (
    <div style={{ marginTop: 6, marginBottom: 6 }}>
      ...
      >
        <Bot size={13} color="var(--cc-text-muted)" style={{ flexShrink: 0 }} />
        <span>{label}</span>
      ...
      {expanded && (
        ...
        >
          {events.map((event) => {
            const instructions = truncateAgentInstructions(event.instructions);
            return (
              <div
                key={`${event.kind}-${event.entryId}`}
                style={{
                  overflow: "hidden",
                  textOverflow: "ellipsis",
                  whiteSpace: "nowrap",
              ...
              >
                {isCreated ? "Created " : "Closed "}
                <strong style={{ color: "var(--cc-text-secondary)", fontWeight: 700 }}>
                  {event.agentName}
                </strong>
                {isCreated && instructions && (
                  <span> with instructions: {instructions}</span>
                )}
                {!isCreated && event.outcome && (
...
type RenderItem =
  ...
  | { kind: "agent-lifecycle"; events: SubagentActivityEvent[] }
...
function groupEntries(
  entries: LogEntry[],
  subagentEventsByEntryId: Map<string, SubagentActivityEvent> = new Map(),
): RenderItem[] {
  ...
  let currentAgentEvents: SubagentActivityEvent[] = [];
  ...
  const flushAgentEvents = () => {
    if (currentAgentEvents.length > 0) {
      items.push({ kind: "agent-lifecycle", events: [...currentAgentEvents] });
      currentAgentEvents = [];
    }
  };

  for (const originalEntry of entries) {
    const agentEvent = subagentEventsByEntryId.get(originalEntry.id);
    if (agentEvent) {
      flushTextGroup();
      flushToolBuffer();
      const previousAgentEvent = currentAgentEvents[currentAgentEvents.length - 1];
      if (previousAgentEvent && !shouldGroupSubagentActivityEvents(previousAgentEvent, agentEvent)) {
        flushAgentEvents();
      }
    ...
    if (entry.type === "text" || entry.type === "question") {
      ...
      if (isWorkspacePolicyDenialText(entry.toolResult || entry.content)) {
        flushTextGroup();
        flushToolBuffer();
        flushAgentEvents();
        items.push({ kind: "single", entry });
      }
      // Other tool_result entries are hidden — their content is noise.
      continue;
...
function buildMergedTimeline(
  ...
  const flushParentBuffer = () => {
    if (parentBuffer.length > 0) {
      result.push(...groupEntries(parentBuffer, subagentEventsByEntryId));
      parentBuffer = [];
    }
  };
...
interface ModalChatProps {
  ...
  permissionMode?: PermissionMode;
  /** JSON list of context files available for the startup context notice. */
  ...
  onRefresh?: () => void;
  /** Called after a user message edit creates its internal replacement task. */
...
export function ModalChat({
  ...
  const fetchLogEntries = useTaskStore((state) => state.fetchLogEntries);
  const logLoadStatus = useTaskStore((state) => state.logLoadStatus[taskId] ?? "idle");
  const logLoadError = useTaskStore((state) => state.logLoadErrors[taskId] ?? null);
  ...
  // Keep the latest turn visible while content streams or the composer changes
  // the available viewport. Respect readers who intentionally scrolled up.
  useEffect(() => {
  ...
  const handleScroll = useCallback(() => {
    const el = scrollRef.current;
    if (!el) return;
    const isNearBottom = isFeedChatNearBottom(el);
  ...
  const showLogLoadError = !hasEntries && !hasChildren && logLoadStatus === "error";
  const showEmpty = !hasEntries && !hasChildren && neverStarted && !showLogLoadError;
  const isTerminal = status === "done" || status === "review" || status === "error";
  const showLoading = !hasEntries
    && !hasChildren
    && !neverStarted
    && !isRunning
    && !showLogLoadError
    && (logLoadStatus === "idle" || logLoadStatus === "loading");
  const showNoHistory = !hasEntries
    && !hasChildren
    && isTerminal
    && logLoadStatus === "loaded";
  ...
  const subagentActivity = useMemo(
    () => buildSubagentActivityModel(displayLogEntries),
    [displayLogEntries],
  );
  const activeSubagentCount = subagentActivity.activeCount;
  ...
  const renderItems = useMemo(() => {
    const grouped = hasChildren
      ? buildMergedTimeline(displayLogEntries, childTasks, childLogEntries, subagentActivity.eventsByEntryId)
      : groupEntries(displayLogEntries, subagentActivity.eventsByEntryId);
    ...
    const TURN_BREAK_KINDS = new Set(["single", "agent-lifecycle", "child-question", "child-review", "phase-divider"]);
    ...
    return result;
  }, [hasChildren, displayLogEntries, childTasks, childLogEntries, isRunning, subagentActivity.eventsByEntryId]);
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        data-feed-chat-viewport={feedPresentation ? "true" : undefined}
        role="log"
      ...
      >
        <div
          ref={contentRef}
          data-feed-chat-rail={feedPresentation ? "true" : undefined}
          style={{
            width: feedPresentation ? "min(720px, calc(100% - 48px))" : "100%",
            minHeight: "100%",
            margin: feedPresentation ? "0 auto" : undefined,
            padding: feedPresentation ? "20px 0 8px" : "24px 24px 16px 24px",
            display: "flex",
            flexDirection: "column",
            gap: 6,
            minWidth: 0,
        ...
        {showLogLoadError && (
          ...
          >
            <span style={{ fontSize: 13, color: "var(--cc-status-danger)", fontFamily: "var(--cc-font-body)" }}>
              {logLoadError ?? "Conversation history could not be loaded."}
            </span>
            <button
              type="button"
              onClick={() => { void fetchLogEntries(taskId); }}
              style={{
                height: 28,
                padding: "0 10px",
        ...
        {renderItems.map((item, index) => {
          ...
          if (item.kind === "agent-lifecycle") {
            return (
              <AgentLifecycleBlock
                key={`agent-${index}-${item.events[0]?.entryId ?? "empty"}`}
                events={item.events}
                activeKeys={subagentActivity.activeKeys}
              />
            );
          }
        ...
        {shouldShowCompletedTaskMarker({ status, isRunning, activeSubagentCount, totalWorkSeconds }) && (
          <CompletedVerb
      ...
      {showJumpButton && hasEntries && (
        <button
          data-feed-chat-jump-latest={feedPresentation ? "true" : undefined}
