...
export function ReviewQueue({ tasks, allTasks }: { tasks: Task[]; allTasks?: Task[] }) {
  ...
  const visibleAllTasks = useMemo(() => getVisibleEditedTasks(allTasks ?? tasks), [allTasks, tasks]);

  const queue = visibleTasks.filter((t) => !markedDone.has(t.id)).sort((a, b) => {
  ...
  const projectContext = useMemo(() => {
    if (!current || !visibleAllTasks) return null;
    if (current.parentId) {
      const parent = visibleAllTasks.find(t => t.id === current.parentId);
      if (parent) {
        const siblings = visibleAllTasks.filter(t => t.parentId === parent.id);
    ...
    if (current.projectSlug) {
      const projectTasks = visibleAllTasks.filter(t => t.projectSlug === current.projectSlug && t.parentId);
      if (projectTasks.length > 0) {
        const parent = visibleAllTasks.find(t => t.projectSlug === current.projectSlug && !t.parentId);
        const done = projectTasks.filter(t => t.status === "done").length;
        return { name: parent?.title || current.projectSlug, done, total: projectTasks.length };
      }
    }
    return null;
  }, [current, visibleAllTasks]);
